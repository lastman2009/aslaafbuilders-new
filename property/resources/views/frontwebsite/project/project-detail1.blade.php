...
<section class="page-section pbbottom ptop">
    <div class="col-md-12 col-sm-12 col-xs-12 pleft features">
        <figure class="pull-left home-icon"><img src="/assets/images/home-icon.jpg"> </figure>
        <div class="feature-heading pull-left" style="margin-bottom: 0">
            <h2>Property<span> Types</span></h2>
            <p>Pakistan's Best Property Portal.</p>
        </div>
        @foreach($data['scheme'] as $scheme)
        <div class="col-md-6 prop-types no-padding">
            <div class="Prop-heading">
                <h4>{{$scheme->title}}<span class="pull-right">{{ $scheme->property_type_name }}</span></h4>
            </div>
            <ul class="list-unstyled">
                <li><img src=""><span>Area</span><span class="pull-right">{{$scheme->area}} {{$scheme->area_type}}</span></li>
                @if($scheme->bed != 0)
                <li><img src=""><span>Bed</span><span class="pull-right">{{$scheme->bed}}</span></li>
                @endif
                @if($scheme->bath != 0)
                <li><img src=""><span>Bath</span><span class="pull-right">{{$scheme->bath}}</span></li>
                @endif
                <li><img src=""><span>Min Price</span><span class="pull-right">{{$scheme->min_price}}</span></li>
                <li><img src=""><span>Max Price</span><span class="pull-right">{{$scheme->max_price}}</span></li>
            </ul>
        </div>
        @endforeach
...
@if(!$data['payment_plan']->isEmpty())

        <section class="payments-sect">
                                <div class="col-md-12 col-sm-12 col-xs-12 pleft features">
                                    
                                    <figure class="pull-left home-icon"><img src="/assets/images/project-plan-icon.png"> </figure>
                                    <div class="feature-heading pull-left" style="margin-bottom: 0">
                                        <h2>Payment<span> Plan</span></h2>
                                        <p>Pakistan's Best Property Portal.</p>
                                    </div>
                                                    ...
                                                    <ul class="list-unstyled list-inline">
                                                        ...
                                                        <li> 
                                                            <a href="/images/property/paymentPlan/original_{{ $payment->image }}" class="lightbox_trigger_1234">
                                                                <figure><img src="/images/property/paymentPlan/thumb_{{ $payment->image }}" class="img-responsive"></figure>
                                                            </a>
                            ...
                            @endif
@if(!$data['floor_plan']->isEmpty())
                            <section class="floor-plan-sect">
                                <div class="col-md-12 col-sm-12 col-xs-12 pleft features">
                                    <figure class="pull-left home-icon"><img src="/assets/images/floor-plan-icon.png"> </figure>
                                    <div class="feature-heading pull-left" style="margin-bottom: 0">
                                        <h2>Floor<span> Plan</span></h2>
                                        <p>Pakistan's Best Property Portal.</p>
                                    </div>
                                    <div class="floor-plan-back">
                                        <div id="lightboxfloorplan">
                                            <div id="wrapper_1234567">
                                                <p>
                                                    <ul class="list-unstyled list-inline">
                                                        @foreach($data['floor_plan'] as $floor) 

                                                        <li> 
                                                            <a href="/images/property/floorPlan/original_{{ $floor->image }}" class="lightbox_trigger_1234567">
                                                                <figure><img src="/images/property/floorPlan/thumb_{{ $floor->image }}" class="img-responsive"> </figure>
                                                            </a>
                                                            <p>Content Goes Here</p>
...
$('.lightbox_trigger_1234').click(function (e) {
        e.preventDefault();
        // getting and storing image href
        var image_href = $(this).attr("href");
        // if #lightbox exists
        if ($('#lightbox_1234').length > 0) {
            // inserts img tag with stored href
            $('#content_abc').html('<img src="' + image_href + '" />');
            // show lightbox window
            $('#lightbox_1234').fadeIn();
        } else { //if #lightbox does not exist
            // stores lightbox html content
            var lightbox_abc =
                '<div id="lightbox_1234">' +
                '<span class="lightbox_close_btn"><i class="fa fa-times"></i></span>' +
                '<div id="content_abc">' +
                '<img src="' + image_href + '" />' +
                '</div>' +
                '</div>';
            // inserts lightbox html content into page
            $('body').append(lightbox_abc);
        }
        // on click, hide lightbox window
        $('#lightbox_1234').on('click', function () {
            $('#lightbox_1234').fadeOut();
        });
    });


$('.lightbox_trigger_1234567').click(function (e) {
        e.preventDefault();
        // getting and storing image href
        var image_href_123 = $(this).attr("href");
        // if #lightbox exists
        if ($('#lightbox_1234567').length > 0) {
            // inserts img tag with stored href
            $('#content_1234').html('<img src="' + image_href_123 + '" />');
            // show lightbox window
            $('#lightbox_1234567').fadeIn();
        } else { //if #lightbox does not exist
            // stores lightbox html content
            var lightbox_abcdef =
                '<div id="lightbox_1234567">' +
                '<span class="lightbox_close_btn"><i class="fa fa-times"></i></span>' +
                '<div id="content_1234">' +
                '<img src="' + image_href_123 + '" />' +
                '</div>' +
                '</div>';
            // inserts lightbox html content into page
            $('body').append(lightbox_abcdef);
        }
        // on click, hide lightbox window
        $('#lightbox_1234567').on('click', function () {
            $('#lightbox_1234567').fadeOut();
        });
    });
